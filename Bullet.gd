...
func _ready():
	...
	# Force collision mask to detect ALL physics bodies (Players, Enemies, Buildings, Base)
	collision_layer = 0
...
func _draw() -> void:
	if is_enemy_bullet:
		# Luminous Orange/Yellow Scrap Slug
...
func _on_body_entered(body: Node2D):
	...
	if multiplayer.is_server() or not multiplayer.has_multiplayer_peer():
		if is_enemy_bullet:
			# Enemy Bullets ONLY damage Players, Bodyguards, Buildings, and Base
			...
			# Friendly Bullets ONLY damage Enemies and Waaagh Idols
