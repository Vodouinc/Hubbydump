...
@export var ferro_plate_base: Color = Color(0.11, 0.13, 0.16, 1.0)     # Outer Heavy Ferrocrete
@export var ferro_plate_inner: Color = Color(0.17, 0.19, 0.23, 1.0)    # Inset Plasteel Deck
@export var plate_bevel_light: Color = Color(0.42, 0.48, 0.55, 1.0)    # Polished Steel Outer Rim
@export var plate_bevel_dark: Color = Color(0.06, 0.07, 0.09, 1.0)     # Inset Seam Shadow
@export var brass_trim_color: Color = Color(0.78, 0.58, 0.22, 1.0)     # Mechanicus Brass Trim
...
# Cached procedurally merged territory geometry
var cached_slab_polygons: Array[PackedVector2Array] = []
var cached_inner_polygons: Array[PackedVector2Array] = []

func _ready() -> void:
	z_index = -10  # Ensures the sand & metal slab are always drawn beneath all unit/building shadows
...
func _process(delta: float) -> void:
	anim_timer += delta
	# Redraw for the flowing conduit energy animation
	queue_redraw()
...
## Procedurally merges all structure foundations and corridors into a single continuous mega-slab
func refresh_foundations() -> void:
	var active_structures = _gather_active_structures()
	
	if active_structures.is_empty():
	...
	# 1. Circle approximation polygons for each building
	...
	# 2. Corridors bridging close structures into the slab
	for i in range(active_structures.size()):
		for j in range(i + 1, active_structures.size()):
			var s1 = active_structures[i]
			var s2 = active_structures[j]
	...
	# 3. Boolean union: fuse all overlapping shapes into unified polygon islands
	cached_slab_polygons = _fuse_polygons(raw_polygons)

	# 4. Generate inset decking for each merged island (slight inner margin)
	cached_inner_polygons.clear()
...
func _draw() -> void:
	...
	# 2. DRAW PROCEDURAL MERGED INDUSTRIAL ART SLAB
	# 2A. Drop Shadows
	for slab in cached_slab_polygons:
	...
	# 2B. Outer Solid Ferrocrete Deck
	for slab in cached_slab_polygons:
		draw_colored_polygon(slab, ferro_plate_base)

	# 2C. Inset Plasteel Interior Plate
	...
	# 2D. Crisp Outer Perimeter Edge (Single Continuous Border, Zero Internal Overlaps)
	for slab in cached_slab_polygons:
	...
	# 3. DRAW POWER CONDUITS & ENERGY FLOW
	var active_structures = _gather_active_structures()
...
func _draw_conduit_cable(p1: Vector2, p2: Vector2, radius_1: float, radius_2: float) -> void:
	...
	var segment_len = start.distance_to(finish)
	if segment_len < 2.0: return
	...
	# Animated outward flowing energy pulses
	...
	while cur_dist < segment_len:
		var pulse_pos = start + dir_norm * cur_dist
		draw_circle(pulse_pos, 2.5, conduit_glow_color)
